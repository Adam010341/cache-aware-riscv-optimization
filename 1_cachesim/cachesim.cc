...
void cache_sim_t::update_tree_on_hit(size_t idx, size_t way)
{
  for(size_t node=ways+way;node>1;node/=2){
    plru_tree[idx*ways+node/2]=(node%2==0);
  }
}
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  size_t node=1;
  while(node<ways){
    uint8_t &dir=plru_tree[idx*ways+node];
    node=node*2+dir;
    dir^=1;
  }
  size_t way=node-ways;
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  if (likely(hit_way != NULL))
  {
    ...
    size_t idx = (addr >> idx_shift) & (sets-1);
    size_t way = hit_way - &tags[idx * ways];
    update_tree_on_hit(idx, way);
